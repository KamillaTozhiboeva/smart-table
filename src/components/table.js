...
export function initTable(settings, onAction) {
    ...
    const nextRows = data.map((item) => {
            ...
            Object.keys(item).forEach((key) => { //перебираем все ключи
                if (row.elements[key]) { 
                    const el = row.elements[key];
                    if (el.tagName === "INPUT" || el.tagName === "SELECT") {
                        el.value = item[key];
                    } else {
                        el.textContent = item[key]; //проверяем, чтобы инпут или селект был инпутом и селектом. Если тру, то присваем значение в value, 
                    }
                }
            });

            return row.container; //возвращаем row с conteiner для map 
        });
